...
struct EinsumNode : expr {
 /*
 indexes; list of einsum indices: Eg: einsum("ij,jk,kl -> il" , A,B,C) would return an EinsumNode object with indices string "ij","jk","ik","il"
 inputs: list of tensor objects to be operated on.
 */
vector<string> indexes;
vector<Tensor> inputs;
EinsumNode(const vector<string> &idx, const std::vector<Tensor>& inp_tensors){
if (idx.size() == inp_tensors.size() + 1){
indexes = idx;
inputs = inp_tensors;
 }
else {
throw std::invalid_argument("Input indices don't match the number of inputs provided.");
 }
 }
};
...
struct shape_variable{
 /*
 Helper struct that represents a shape variable used to store the range of each loop in Einsum calculation.
 */
...
struct LoopNode: IRNode{
 ...
 EisnumNode("ij","ji",A) would have LoopNode(i, A[0], LoopNode(j,A[1], Calcnode(temp1[j][i] = A[i][j]) ))
 BinOpNode(Multiply, A,B) would have LoopNode()
 */
char loop_variable;
...
void generateCode(shared_ptr<IRNode> node, int indent = 0) {
    ...
    if (auto loopNode = dynamic_pointer_cast<LoopNode>(node)) {
        cout << indentStr << "for (int " << loopNode->loop_variable 
             << " = 0; " << loopNode->loop_variable << " < " 
             << loopNode->shape_var.tensor_name->name << ".shape[" 
             << loopNode->shape_var.idx_posn << "]; " 
             << loopNode->loop_variable << "++) {" << endl;
...
shared_ptr<IRNode> convertASTtoIR(shared_ptr<expr> AST) {
    ...
    if (auto binOpPtr = dynamic_pointer_cast<BinOpNode>(AST)) {
        ...
        bool rightIsTensor = dynamic_pointer_cast<Tensor>(binOpPtr->inp2) != nullptr;
        
        if (leftIsTensor && rightIsTensor) {
            throw runtime_error("Binary operations between two tensors not yet implemented");
        }
        ...
        if (rightIsTensor) {
            ...
            string calcCode = tempName + " = prev_result" + opStr + rightTensor->name + ";";
...
int main (){
    ...
    auto prog_AST = std::make_shared<BinOpNode>(Add, einsumNode3, std::make_shared<Tensor>("C"));
